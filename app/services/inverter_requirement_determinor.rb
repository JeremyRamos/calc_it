module InverterRequirementDeterminor
  ...
  BATTERY_CAPACITY = 52 # calculations is done on a 105 Ah battery, only using 50%

  def call(total_wattage)
    return [0, 0, 0] if total_wattage.to_s.start_with?('0')

    [calculate_inverter_size(total_wattage), *calculate_battery_duration(total_wattage)]
  end
  ...
  def calculate_battery_duration(total_wattage, efficiency = 0.8, voltage = 12)
    ...
    duration_in_hours = BATTERY_CAPACITY / amp_hours
